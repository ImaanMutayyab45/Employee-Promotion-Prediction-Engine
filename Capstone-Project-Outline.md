...
> **Data note (confirmed against the provided `train.csv`):** this working copy of the dataset contains 54,808 rows and 13 columns (`employee_id`, `department`, `region`, `education`, `gender`, `recruitment_channel`, `no_of_trainings`, `age`, `previous_year_rating`, `length_of_service`, `awards_won?`, `avg_training_score`, `is_promoted`). It does **not** include a `KPIs_met >80%` column, even though some public mirrors of this dataset do — flagged so the KPI-met flag in Feature Engineering (step 2 below) is adjusted accordingly. 
